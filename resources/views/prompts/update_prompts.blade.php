            ...
            <div class="">
                <div class=" d-flex flex-wrap align-items-center px-0 pt-0">
                    @auth
                        ...
                        <form action="{{ route('users.topics.manage', [auth()->user(), $prompt->topic->id]) }}" method="get" class="mr-1">
                        @csrf
        ...
        @if ($prompt)
            <div class="mb-4">
                <div class="container-fluid">
                    <h4 class="display-7 py-1 fw-bold ">{{ $prompt->topic-> topic}}</h4>
                    <p style="font-size: 14px"><strong class="text-primary">Description </strong><br> {{ $prompt->topic-> description}}</p>
                    <p><small>hosted by</small> <small class="text-info">{{ $prompt-> user->name}}</small></p>
                </div>
            </div>
        @endif
        <div class="d-flex justify-content-between align-items-center">
            <div class="sc-fUqQNk jDAUBC avatar-group--dense">
                <img width="20" height="20" class="rounded-circle flex-shrink-0" class="" src="/images/-min-29.jpg" prompt="Abhishek Kumar" alt="r">
                <img width="20" height="20" class="rounded-circle flex-shrink-0" class="sc-jtmhnJ jpjECk" src="/images/897193_small500.png" prompt="Jason Sykes" alt="s">
                <img width="20" height="20" class="rounded-circle flex-shrink-0" class="sc-jtmhnJ jpjECk" src="/images/cxc.jpg" prompt="Ajith Pushparaj" alt="j">
            </div>
            <form action="{{ route('topics.details', [$prompt->topic_id]) }}" method="get" >
            @csrf
            ...
            <div class="row g-5 py-3 mx-2 justify-content-center">
                ...
                <div class="col-md-12 col-lg-12">
                    <form class="row g-3 needs-validation d-flex flex-row align-items-center justify-content-center"
                        novalidate action="{{ route('users.updateprompts',  [auth()->user(), $prompt]) }}" method="post" enctype="multipart/form-data">
                        
                        @csrf
                        <div class="col-12">
                            <h5 class="py-4 fw-bold text-info">Update Prompts</h5>
                        </div>

                        <div class="col-md-12">
                            <label for="question" class="form-label">question</label>
                            <input type="text" name="question" id="question" placeholder="question" 
                            class="form-control py-2  rounded-lg @error('question') border border-danger @enderror" value="{{ $prompt->question }}">
                        ...
                        <div class="col-md-12">
                            ...
                            <a href="{{ route('users.topics.manage', [auth()->user(), $prompt->topic->id]) }}" class="btn btn-danger">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
